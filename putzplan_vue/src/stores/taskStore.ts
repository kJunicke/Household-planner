...
export const useTaskStore = defineStore('tasks', () => {
  ...
  const estimateCompletionEffort = (task: Task, effortOverride?: number): number => {
    ...
    const deductSum = tasks.value
      .filter(
        (t) =>
          t.parent_task_id === task.task_id && t.completed && t.subtask_points_mode === 'deduct',
      )
  ...
  const markAsDirty = async (taskId: string) => {
    ...
    const { error } = await serializeMutation(
      taskId,
      async () =>
        await supabase
          .from('tasks')
  ...
  const createTask = async (
    taskData: Partial<Task> & Pick<Task, 'title' | 'effort' | 'recurrence_days' | 'task_type'>,
  ) => {
    const householdStore = useHouseholdStore()
    const toastStore = useToastStore()
  ...
  const subscribeToTasks = () => {
    ...
    console.log(
      '🔴 Subscribing to tasks for household:',
      householdStore.currentHousehold.household_id,
    )
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'DELETE') {
            ...
            completions.value = completions.value.filter(
              (c) => c.completion_id !== deletedCompletion.completion_id,
            )
  ...
  const fetchCompletions = async () => {
    ...
    const enriched = filteredData.map((completion) => {
      ...
      return {
        ...
        household_members: {
          display_name:
            householdStore.householdMembers.find((m) => m.user_id === completion.user_id)
