...
const overdueColorStyle = computed(() => {
  ...
  const intensity =
    schedule.value.status === 'never-done'
      ? 1
...
const handleCreateSubtask = async (subtaskData: {
  ...
  const maxOrderIndex = subtasks.value.reduce(
    (max: number, s: Task) => Math.max(max, s.order_index),
    0,
  )
...
const projectEffort = computed(() =>
  isProject.value ? taskStore.getProjectEffort(props.task.task_id) : 0,
)
...
<template>
  <div
    ...
    :style="assignmentTintStyle"
  >
    <!-- Main Horizontal Layout -->
    <div
      class="task-card-main"
      @click="!props.task.parent_task_id && subtasks.length > 0 ? toggleSubtasks() : null"
      :style="{ cursor: !props.task.parent_task_id && subtasks.length > 0 ? 'pointer' : 'default' }"
    >
      ...
      <div class="task-left">
        <div class="task-info-block">
          ...
          <div class="task-meta">
            ...
            <span v-else class="meta-badge effort-badge"> {{ projectEffort }} Pkt </span>
      ...
      <div class="task-right" @click.stop.prevent>
        ...
        <template v-if="isProject">
          ...
          <div v-else class="completed-badge">✓</div>
        </template>
    ...
    <div
      v-if="!props.task.parent_task_id && subtasks.length > 0 && subtasksExpanded"
    ...
    >
      ...
      <div class="subtasks-list">
        ...
        <template v-if="props.task.task_type === 'daily'">
          <SubtaskItem v-for="subtask in subtasks" :key="subtask.task_id" :task="subtask" />
        </template>
